...
public class MainActivity extends BaseActivity implements ATEActivityThemeCustomizer {
    ...
    SlidingUpPanelLayout panelLayout;//主页面上拉view
    @BindView(R.id.nav_view)
    NavigationView navigationView;//侧滑菜单项
    @BindView(R.id.drawer_layout)
    DrawerLayout mDrawerLayout;//侧滑容器
    ...
    */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        action = getIntent().getAction();//android.intent.action.MAIN
        ...
        navDrawerRunnable.postDelayed(new Runnable() {
            @Override
            public void run() {
                setupDrawerContent(navigationView);//设置菜单监听
                setupNavigationIcons(navigationView);//设置菜单图标
            }
        }, 700);

        //判断是否 6.0以上系统
        if (ListenerUtil.isMarshmallow()) {
            checkPermissionAndThenLoad();//动态申请权限
        } else {
            loadEverything();
        }
